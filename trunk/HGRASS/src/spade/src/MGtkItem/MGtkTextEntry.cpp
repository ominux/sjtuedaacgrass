...
void MGtkTextEntry::getXmlNodeProperties(xmlNode* node)
{
  ...
  while(attPtr != NULL)
  {
    ...
    else if(!xmlStrcmp(attPtr->name, sTEMaxLen_IB))
      this->maxLength = atoi((char*)value);
...
bool MGtkTextEntry::isValidEvent(MGtkEvent* pevent)
{
  ...
  if(   strcmp(pevent->event,"activate")
     ...
     && 1/*ToDo*/)
    return false;
  if(   strcmp(pevent->func,"MGtkTextEntryActivate")
     && strcmp(pevent->func,"MGtkTextEntryChanged")
     &&1/*ToDo*/)
    return false;
  return true;
}

void MGtkTextEntry::genGtkGUI()
{
   ptr = gtk_entry_new();
   if(isFixedSize)
   ...
   if(label)
     gtk_entry_set_text(GTK_ENTRY(ptr),label);
   gtk_entry_set_editable(GTK_ENTRY(ptr),editable);
   gtk_entry_set_has_frame(GTK_ENTRY(ptr),hasFrame);
   gtk_entry_set_max_length(GTK_ENTRY(ptr),maxLength);
   gtk_entry_set_width_chars(GTK_ENTRY(ptr),charWidth);
   /*if(label)*/
   ...
   if(numEvents != 0)
   {
	 ...
	 GCallback pGtkFunc = NULL;
	   ...
	   else if(!strcmp(pevent->func, "MGtkTextEntryActivate"))
	     pGtkFunc = G_CALLBACK(MGtkTextEntryActivate);
	   else if(!strcmp(pevent->func, "MGtkTextEntryChanged"))
	     pGtkFunc = G_CALLBACK(MGtkTextEntryChanged);
           else
             ;//ToDo
         }
         else
