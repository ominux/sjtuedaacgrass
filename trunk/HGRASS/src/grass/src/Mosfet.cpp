...
void MosfetModelBsim3V3::calBasicModelParameters(MosfetModelBsim3V3* pModel, double val, double w, double l)
{
  ...
  calKPVOV(pModel,val,w,l);
  calLAMVOV(pModel,val,w,l);
...
#ifdef DEBUG
  if(isNMOS)
    printf("[DEBUG] The Original Value OF GM is %g, KPVOVn is %g",gm->value,KPVOVn);
  else
    printf("[DEBUG] The Original Value OF GM is %g, KPVOVp is %g",gm->value,KPVOVp);
#endif
  gm->value = KPVOV*val;
#ifdef DEBUG
...
#ifdef DEBUG
  if(isNMOS)
    printf("[DEBUG] The Original Value OF RDS is %g, KPVOVn is %g LAMVOVn is %g",rds->value,KPVOVn,LAMVOVn);
  else
    printf("[DEBUG] The Original Value OF RDS is %g, KPVOVp is %g LAMVOVp is %g",rds->value,KPVOVp,LAMVOVp);
#endif
...
void MosfetModelBsim3V3::calKPVOV(MosfetModelBsim3V3* pModel, double val, double w, double l)
{
  ...
  if(pModel->gm == NULL)
    return;
  double KPVOV = pModel->gm->value / val;
  if(pModel->isNMOS)
  { 
...
void MosfetModelBsim3V3::calLAMVOV(MosfetModelBsim3V3* pModel, double val, double w, double l)
{
  ...
  if(pModel->gm == NULL || pModel->rds == NULL)
    return;
  double LAMVOV = pModel->gm->value * pModel->rds->value;
  if(pModel->isNMOS)
  {
