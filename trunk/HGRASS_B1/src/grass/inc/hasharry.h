...
public:
 ...
 ~BDDHasherArray();        // clear hashers
  ...
  void    Print ( std::ostream& out = cout);        // print the contents
  friend std::ostream& operator << (std::ostream &out, BDDHasherArray f) { f.Print(out); return out; }
  friend std::ostream& operator << (std::ostream &out, BDDHasherArray* f)
                      { out << "(ptr->)"; f->Print(out); return out; }

  void    ShortPrint ( std::ostream& out = cout);    // just stats
