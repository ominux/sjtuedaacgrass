...
class HasherNode : public Object 
{
 ...
 ~HasherNode() {}
  ...
  unsigned Hashval()            { return hashval; }
...
class HasherTree : public Object 
{
  ...
  virtual void PrintNode(HasherNode *, int, std::ostream& out = std::cout);
  void    CheckLevel(HasherNode *, unsigned int);
...
protected:
  ...
  HasherNode *root;
  struct {
    unsigned int max_depth    : 30;	// 30 bits
    boolean    del_data    :  1;	//  1 bit
    boolean    del_key     :  1;
  } flags;
...
public:
 ...
 ~HasherTree();        
  ...
  void SetDelete(boolean d, boolean k) \
  			{ flags.del_data = d; flags.del_key = k; }
  ...
  boolean Get (Object **o, Comparable *key) \
  		{ return 0 != ( *o=Get( key));}  // returns TRUE if in tree

  boolean Remove( Comparable *key);
  void    Rehash();
  void    Print ( std::ostream& out = std::cout) ;       // print the contents
  void    ShortPrint ( std::ostream& out = std::cout);  // just stats
  integer isa() { return HASHER_TREE; }
};

inline std::ostream& operator << (std::ostream &out, HasherTree& f)
...
public:
 ...
 ~Hasher_iterator() {;}
  ...
  void        FindBottom();  
  // find a node with null pointers, "push"ing nodes on the way
...
public:
 ...
 ~Hasher_updater() {;}
  ...
  boolean  Update( Object* o)     \
  	{ if (n!=NULL) n->data = o; return( n!=NULL); }
};
