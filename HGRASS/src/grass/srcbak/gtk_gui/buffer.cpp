...
#pragma omp parallel for schedule(static)
	...
	double tEnd = clock();
	cout<<"FR with 100 Point timing "<<1.0*(tEnd-tStart)/1000000 << endl;
}
...
void Buffer::SaveToFile(ofstream &fileName)
{
	for (int i = 0; i < num_points_; ++i)
	{
		fileName << setw(10) << setprecision(10) << pow(10.0, freq_vector_[i]);
		fileName << setw(20) << setprecision (10)
			<< setiosflags(ios::fixed) << magnitude_vector_[i];
		fileName << setw(20) << setprecision (10)
			<< setiosflags(ios::fixed) << phase_vector_[i] << endl;
	}
}
